...
},{"./BuonduaParser":57,"paperback-extensions-common":13}],57:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
const XiutakuParser_1 = require("./XiutakuParser");
...
exports.XiutakuInfo = {
    ...
    contentRating: paperback_extensions_common_1.ContentRating.ADULT,
    websiteBaseURL: XiutakuParser_1.X_DOMAIN,
    sourceTags: [
        {
            text: '18+',
            type: paperback_extensions_common_1.TagType.RED
...
class Xiutaku extends Buondua_1.Buondua {
    constructor() {
        super(...arguments);
        this.requestManager = createRequestManager({
            requestsPerSecond: 4,
            ...
            interceptor: {
                interceptRequest: async (request) => {
                    request.headers = {
                        ...(request.headers ?? {}),
                        ...{
                            'referer': XiutakuParser_1.X_DOMAIN
                        }
                    };
                    return request;
                },
    ...
    getMangaShareUrl(mangaId) {
        return `${XiutakuParser_1.X_DOMAIN}/${mangaId}`;
    }
    async getHomePageSections(sectionCallback) {
        const requestForRecent = createRequestObject({
            url: `${XiutakuParser_1.X_DOMAIN}`,
            method: 'GET'
        });
        const responseForRecent = await this.requestManager.schedule(requestForRecent, 1);
        const $recent = this.cheerio.load(responseForRecent.data);
        const recentAlbumsSection = createHomeSection({ id: 'recent', title: 'Recently Uploaded', view_more: true, type: paperback_extensions_common_1.HomeSectionType.singleRowNormal });
        const recentAlbums = (0, XiutakuParser_1.getAlbums)($recent);
        recentAlbumsSection.items = recentAlbums;
        sectionCallback(recentAlbumsSection);
        const requestForHot = createRequestObject({
            url: `${XiutakuParser_1.X_DOMAIN}/hot`,
            method: 'GET'
        });
        const responseForHot = await this.requestManager.schedule(requestForHot, 1);
        const $hot = this.cheerio.load(responseForHot.data);
        const hotAlbumsSection = createHomeSection({ id: 'hot', title: 'Hot', view_more: true, type: paperback_extensions_common_1.HomeSectionType.singleRowNormal });
        const hotAlbums = (0, XiutakuParser_1.getAlbums)($hot);
        hotAlbumsSection.items = hotAlbums;
        sectionCallback(hotAlbumsSection);
    }
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        const albums = (0, XiutakuParser_1.getAlbums)($);
        metadata = !(0, BuonduaParser_1.isLastPage)($) ? { page: albumNum + albums.length } : undefined;
        return createPagedResults({
            results: albums,
            metadata
    ...
    async getMangaDetails(mangaId) {
        const data = await (0, XiutakuParser_1.getGalleryData)(mangaId, this.requestManager, this.cheerio);
        return createManga({
            id: mangaId,
            titles: data.titles,
            image: data.image,
    ...
    async getChapterDetails(mangaId, chapterId) {
        return createChapterDetails({
            ...
            mangaId: mangaId,
            longStrip: false,
            pages: await (0, XiutakuParser_1.getPages)(mangaId, this.requestManager, this.cheerio)
        });
    }
    async getSearchResults(query, metadata) {
        const albumNum = metadata?.page ?? 0;
        ...
        if (query.title) {
            request = createRequestObject({
                url: `${XiutakuParser_1.X_DOMAIN}/?search=${encodeURIComponent(query.title)}&start=${albumNum}`,
                method: 'GET'
            });
        }
        else {
            request = createRequestObject({
                url: `${XiutakuParser_1.X_DOMAIN}/${query.includedTags?.map(x => x.id)}?start=${albumNum})`,
                method: 'GET'
            });
        }
        const response = await this.requestManager.schedule(request, 1);
...
},{"../Buondua/Buondua":56,"../Buondua/BuonduaParser":57,"./XiutakuParser":59,"paperback-extensions-common":13}],59:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getPages = exports.getGalleryData = exports.getAlbums = exports.REGEX_ASIAN = exports.X_DOMAIN = void 0;
const entities = require("entities");
exports.X_DOMAIN = 'https://Xiutaku.com';
exports.REGEX_ASIAN = /[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uff66-\uff9f\u3131-\uD79D]/;
function getAlbums($) {
    const albums = [];
    const albumCoverGroups = $('div.blog').toArray();
...
async function getPages(id, requestManager, cheerio) {
    ...
    for (let i = 0; i < pageCount; i++) {
        const request = createRequestObject({
            url: `${exports.X_DOMAIN}/${id}?page=${i + 1}`,
            method: 'GET'
        });
        const data = await requestManager.schedule(request, 1);
        const $ = cheerio.load(data.data);
